chrome.storage.sync.get(["injectedArgs"], (result) => {
    ...
    if (window.brimstomeRecorder !== undefined) {
        ...
        class Recorder {
            ...
            /** If therer is a queued event it is released. Meaning it is now seen by the browser. */
                ...
                switch (msg) {
                    case 'keydown':
                        if (this.pendingEvent.key !== 'Tab') {
                            this.pendingEvent.target.value += this.pendingEvent.key; // '\t' isn't actually in the value for text inputs
                            this.pendingEvent.target.dispatchEvent(new KeyboardEvent('keydown', {
                        ...
                        break;
                    case 'mousedown':
                        this.pendingEvent.target.dispatchEvent(new Event('mousedown', { bubbles: true }));
                        this.pendingEvent.target.focus();
                        this.pendingEvent = null;
                        break;
                    case 'change':
                        ...
                        break;
                    default:
                        console.error(`unexpected completion message from popup: ${this.pendingEvent.type}`);
                        this.enableQueuing();
                        return;
                }
                this.enableQueuing();
        ...
        window.brimstoneRecorder = new Recorder();
        window.brimstonePlayer = new Player();
    }
});
