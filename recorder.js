...
TopLevelObject.DOMPresentationUtils.cssPath = function (node, optimized) {
  ...
  while (contextNode) {
    var step = TopLevelObject.DOMPresentationUtils._cssPathStep(
      contextNode,
      !!optimized,
      contextNode === node
    );
...
TopLevelObject.DOMPresentationUtils._cssPathStep = function (
  node,
  optimized,
  isTargetNode
) {
  ...
  if (optimized) {
    if (id) return new TopLevelObject.DOMNodePathStep(idSelector(id), true);
    ...
    if (
      ...
      return new TopLevelObject.DOMNodePathStep(
  ...
  for (
    ...
    for (var j = 0; j < siblingClassNamesArray.length; ++j) {
      ...
      if (!ownClassNames.hasOwnProperty(siblingClass)) continue;
...
class Recorder {
  ...
  reset() {
    ...
    if (this._frameId === undefined) {
      //  we may reset when recovering from an error, don't lose the frameId
      this._frameId = 0;
    }
  ...
  constructor() {
    ...
    chrome.runtime.onMessage.addListener(
      this._runtimeFrameIdSpecificOnMessageHandler.bind(this)
    ); // extension sends message to one or all frames
    chrome.runtime.onConnect.addListener(
      this._runtimeOnConnectHandler.bind(this)
    ); // extension will connect the port when it is time to start recording
    /** the css for a keyboard mouse cursor */
    this.keyboardCursor = `url(${chrome.runtime.getURL(
      'images/keyboard.png'
    )}) 0 0, not-allowed`;
    /** the css for a mousemove cursor */
    this.mousemoveCursor = `url(${chrome.runtime.getURL(
      'images/mouse.png'
    )}) 15 15, wait`;
  ...
  _runtimeFrameIdSpecificOnMessageHandler(message, sender, sendResponse) {
    console.debug(
      'connect: _runtimeFrameIdSpecificOnMessageHandler',
      message,
      sender
    );
  ...
  postMessageOffsetIntoIframes() {
    console.debug(
      `TX: frame ${this._frameId}:${window.location.href} broadcasts to each child frame their own offset from this frame`
    );
  ...
  clearTimeouts() {
    ...
    clearTimeout(this.waitActionDetectionTimeout);
    this.waitActionDetectionTimeout =
      this.pendingKeyTimeout =
      this.pendingWheelTimeout =
      this.pendingMouseMoveTimeout =
        null;
  ...
  buildMsg(e) {
    ...
    let msg = {
      ...
      boundingClientRect:
        e.boundingClientRect || e.target.getBoundingClientRect(),
  ...
  handleEvent(e) {
    e.brimstoneClass =
      e.timeStamp === SYNTHETIC_EVENT_TIMESTAMP ? 'synthetic' : 'user'; // an event simulated by brimstone
    this.event = e;
    let msg;
    ...
    if (this.messageQueue.length) {
      ...
      if (
        this.messageQueue[0].type === 'pollscreen' ||
  ...
  cancel(e) {
    console.debug(
      `EVENT ${e.type} ${e.brimstoneClass} ${
        e.cancelable ? '*cancelled' : '*un-cancelable'
      } frameId:${this._frameId} ${window.location.href}`,
      e
    );
